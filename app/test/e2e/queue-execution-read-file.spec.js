...
describe('EXECUTION_READ_FILE handling process', () => {
    ...
    it('Consume a EXECUTION_READ_FILE message reads the data on the http file link and pushes it through new DATA message(s) (happy case)', async () => {
    ...
    it('Consume a EXECUTION_READ_FILE message with a URL that redirects follows the redirect, reads the data on the http file link and pushes it through new DATA message(s)', async () => {
        const timestamp = new Date().getTime();

        nock('http://api.resourcewatch.org')
            .get('/dataset')
            .reply(307, null, { location: 'http://api.resourcewatch.org/redirect' });
    ...
    it('Consume a EXECUTION_READ_FILE message reads the data on the https file link and pushes it through new DATA message(s) (happy case)', async () => {
        ...
        const message = {
            ...
            type: 'EXECUTION_READ_FILE',
            taskId: 'efaa80ad-97e4-487b-83ea-a2d0022fc0da',
            fileUrl: 'https://api.resourcewatch.org/dataset',
            dataPath: 'data',
            provider: 'json',
            index: 'index_ef7d64c631664053a0b7e221d84496a5_1574922887909',
            datasetId: `${timestamp}`
    ...
    it('Consume a EXECUTION_READ_FILE message with a URL that redirects follows the redirect, reads the data on the https file link and pushes it through new DATA message(s)', async () => {
        const timestamp = new Date().getTime();

        nock('https://api.resourcewatch.org')
            .get('/dataset')
            .reply(307, null, { location: 'https://api.resourcewatch.org/redirect' });
